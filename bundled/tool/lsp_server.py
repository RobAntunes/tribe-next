...
LSP_SERVER = server.LanguageServer(
    name="MightyDev LSP Server", version="0.1.0", max_workers=MAX_WORKERS
)
...
# Module name for our tool (using a simple available module to avoid errors)
TOOL_MODULE = "json"

# Display name for our tool
TOOL_DISPLAY = "MightyDev Agent IDE"
...
# MightyDev doesn't use traditional linting features
# Instead, we use a simplified LSP implementation

@LSP_SERVER.feature(lsp.TEXT_DOCUMENT_DID_OPEN)
def did_open(params: lsp.DidOpenTextDocumentParams) -> None:
    """LSP handler for textDocument/didOpen request."""
    document = LSP_SERVER.workspace.get_document(params.text_document.uri)
    # We don't publish any diagnostics here - empty list
...
# MightyDev doesn't use traditional formatting features
# Instead, we use AI agents to handle code transformations

@LSP_SERVER.feature(lsp.TEXT_DOCUMENT_FORMATTING)
def formatting(params: lsp.DocumentFormattingParams) -> list[lsp.TextEdit] | None:
    """LSP handler for textDocument/formatting request."""
    # We don't provide formatting through the LSP server
    # Our AI agents handle this through other mechanisms
    return None

# We keep these utility functions as they might be useful elsewhere
